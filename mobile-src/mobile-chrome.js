export async function mountMobileChrome(){
 ...
 async function showUpdates(){const {body,status}=open('App updates');const r=await shell.checkUpdates();body.append(E('p','Installed '+config.nativeVersion+'. iPhone and iPad updates come through the Apple App Store so signing and rollback stay under the store account.'));body.append(E('p',r.url?'Store listing configured.':'The store listing URL will be configured after the first approved release.'));body.append(button('Open store listing',async()=>{busy=true;try{await shell.installUpdate();status.textContent='Opened the store.';}finally{busy=false;}}));}
