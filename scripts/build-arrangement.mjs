...
const out=path.join(root,'dist','arrangement');
...
try{await fs.access(path.join(root,'dist','index.html'));}catch{throw Error('Canonical studio build missing dist/index.html. Run the Vite studio build first.');}
...
console.log('Prepared Core 5 arrangement payload in dist/arrangement/');
